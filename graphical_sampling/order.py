...
class Order:
    ...
    def _get_strategy_order(self, points, strategy, topological=False, grid_dims=None):
        ...
        if grid_dims is not None:
            ...
            num_cols = max(1, points.shape[0] // num_rows)
        ...
        if topological:
            ...
            y_idx = np.argsort(pts[:, 1])
            for row_id in range(num_rows):
                start = row_id * num_cols
                end   = (row_id + 1) * num_cols
                pts[y_idx[start:end], 1] = row_id

            x_idx = np.argsort(pts[:, 0])
            for col_id in range(num_cols):
                start = col_id * num_rows
                end   = (col_id + 1) * num_rows
                pts[x_idx[start:end], 0] = col_id

        # =========================================================
        # HILBERT
        # =========================================================

        if strategy in ['hilbert', 'hilbert_strong']:
            ...
            seed = np.random.randint(0, 4) if strategy == 'hilbert_strong' else 0
        ...
        if strategy == 'morton_grts':
            ...
            for i in range(pts.shape[0]):
                ...
                m_idx = (
                    (part1by1(int(pts[i, 1])) << 1)
                    | part1by1(int(pts[i, 0]))
                )
        ...
        if strategy in ['snake', 'snake_refined', 'snake_staggered']:
        ...
        if strategy == 'snake_move':
            ...
            cols = np.array_split(x_idx, num_cols)
            ...
            for pair_start in range(0, num_cols, 2):
                ...
                row_range = (
                    range(num_rows - 1, -1, -2)
                    if is_descending
                    else range(0, num_rows, 2)
                )
                ...
                for r_start in row_range:
                    ...
                    for target_c, target_r in square_coords:
                        ...
                        for idx in range(num_pts):
                            ...
                            if (
                                logical_grid[idx, 0] == target_c
                                and logical_grid[idx, 1] == target_r
                            ):
